...
class GhostSearch {
  ...
  displayResults(data) {
    if (
      document.querySelectorAll(this.results)[0].firstChild !== null &&
      ...
      while (document.querySelectorAll(this.results)[0].firstChild) {
        document
          ...
          .removeChild(document.querySelectorAll(this.results)[0].firstChild);
      }
    }
    ...
    for (let key in results) {
      if (key < results.length) {
        document
          .querySelectorAll(this.results)[0]
          .appendChild(
            this.createElementFromHTML(this.template(results[key].obj))
          );
      }
    }
